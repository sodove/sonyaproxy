...
- **Поиск** — при поиске в клиенте результаты дополняются треками из YouTube и SoundCloud. Выбираешь трек — он скачивается на лету и стримится. Работает только если клиент использует `search3` для поиска, а не только для индексации библиотеки (Symfonium, например, использует `search3.view` — поддерживается).
